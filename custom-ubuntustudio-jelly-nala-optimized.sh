...
echo "Install nala package manager and basic utilities..."
...
sudo apt update && sudo apt install nala curl git gpg wget -y
...
echo ""
rm -rf $tmp_dir/timeshift-autosnap-apt
git clone https://github.com/wmutschl/timeshift-autosnap-apt.git $tmp_dir/timeshift-autosnap-apt
cd $tmp_dir/timeshift-autosnap-apt
sudo make install

echo ""
echo "** Installing timeshift-autosnap-apt..."
echo ""
rm -rf $tmp_dir/grub-btrfs
git clone https://github.com/Antynea/grub-btrfs.git $tmp_dir/grub-btrfs
cd $tmp_dir/grub-btrfs
sudo make install
...
echo ""
echo "** Installing zip, unzip, 7zip and flatpak commands..."
echo ""
...
sudo nala install -y \
 ...
 code \
 virtualbox-6.1 \
 zip unzip p7zip-full flatpak \
 thunderbird-locale-it \
 libreoffice-l10n-it hyphen-it mythes-it libreoffice-help-it hunspell hunspell-it \
 $tmp_dir/zoom_amd64.deb \
 $tmp_dir/Webex.deb \
 $tmp_dir/onlyoffice-desktopeditors_amd64.deb \
 $tmp_dir/ipscan_3.8.2_amd64.deb \
 keepassxc \
 putty putty-doc \
 filezilla \
 openssh-server \
...
echo ""
echo "INTERNET: Installing Firefox flatpak package..."
echo ""
